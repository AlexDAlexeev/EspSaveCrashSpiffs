...
// Global instance EspSaveCrashFs
#ifndef DONT_USE_ESPSAVECRASH_PRE_GLOBAL
EspSaveCrashFs SaveCrashSpiffs;
#endif
 ...
 */
EspSaveCrashFs::EspSaveCrashFs(const String& directory, const String& prefix, const String& suffix, FS& fs)
    : _fs(fs)
...
void EspSaveCrashFs::setLogFileNameParams(const char* directory, const char* prefix, const char* suffix)
{
 ...
 */
void EspSaveCrashFs::_renewLogFiles()
{
...
**/
uint32_t EspSaveCrashFs::_getNumberOfNameMatch(const String& fileName)
{
 ...
 * @endcode
 */
    void EspSaveCrashFs::_getNextFileName(char* nextFileName, bool findNextName /* = true*/)
{
    nextFileName[0] = '\0';
...
void EspSaveCrashFs::_makeFileName(char* fileName, uint32_t fileNumber)
{
 ...
 * @retval     False  Failed
 */
bool EspSaveCrashFs::removeFile(uint32_t fileNumber)
{
    char fileName[255];
    // Construct new file path
    if (fileNumber == 0) {
 ...
 * @retval     False  Failed
 */
bool EspSaveCrashFs::_checkFile(const char* theFileName, const char* openMode)
{
 ...
 */
bool EspSaveCrashFs::readFile(const char* fileName, char* buffer, size_t bufferSize)
{
    buffer[0] = '\0';
 ...
 * @return     Sucess or error accessing the file
 */
bool EspSaveCrashFs::readFile(uint32_t fileNumber, char* buffer, size_t bufferSize)
{
    char fileName[255];
    // Construct new file path
    if (fileNumber == 0) {
 ...
 * @return     Sucess or error accessing the file
 */
bool EspSaveCrashFs::print(const char* fileName, Print& outputDev)
{
    // check if SPIFFS is working and file exists.
    // NULL parameter as checking reading or writing is not neccessary here
    if (!_checkFile(fileName, "r")) {
 ...
 * @return     Sucess or error accessing the file
 */
bool EspSaveCrashFs::print(uint32_t fileNumber, Print& outputDev)
{
    char fileName[255];
    // Construct new file path
    if (fileNumber == 0) {
...
void EspSaveCrashFs::iterateCrashLogFiles(OnCrashLogFileFound callback)
{
    Dir crashLogDirectory = _fs.openDir(_fileDirectory);
    // Enumerate files in dir
    while (crashLogDirectory.next()) {
